...
namespace Atoms
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("End of Step 1");
            Atom.SerializeFirst("test.txt");
            Console.WriteLine("End of Step 2");
            Atom.DeserializeFirst("test.txt");
            Console.WriteLine("End of Step 3");
            Atom.SerializeElements("test2.txt");
            Console.WriteLine("End of Step 4");
            Atom.DeserializeElements("test2.txt");
            Console.WriteLine("End of Step 5");
        }
    }
    public class Atom
    {
        ...
        public static Atom Parse (string objectData)
        {
            ...
            Atom temp = new Atom(data[0], Convert.ToInt32(data[1]), Convert.ToInt32(data[2]), Convert.ToDouble(data[3]), data[4]);
            return temp;
        }
        ...
        {
            return $"{Name} ({Symbol}) P:{Proton} N:{Neutron} Wt:{Weight}";
        }
        public static void WriteJson(List<Atom> atoms, string filename)
        {
            TextWriter writer = new StreamWriter(filename);
            foreach (Atom i in atoms)
            {
                writer.WriteLine(JsonSerializer.Serialize(i));
            }
            writer.Close();
        }
        public static void ReadJson(string filename)
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    atoms.Add(JsonSerializer.Deserialize<Atom>(temp));
                }
            }
        ...
        {
            foreach(Atom i in GetAtoms())
            {
                Console.WriteLine(i);
            }
        }

        public static void SerializeFirst(string filename)
        {
            TextWriter writer = new StreamWriter(filename);
            writer.WriteLine(JsonSerializer.Serialize(GetAtoms()[0]));
            writer.Close();
        }

        public static void DeserializeFirst(string filename)
        {
            TextReader reader = new StreamReader(filename);
            Atom temp = JsonSerializer.Deserialize<Atom>(reader.ReadLine());
            Console.WriteLine(temp);
            reader.Close();
        }

        public static void SerializeElements(string filename)
        {
            WriteJson(GetAtoms(), filename);
        }

        public static void DeserializeElements(string filename)
        {
            ReadJson(filename);
        }
